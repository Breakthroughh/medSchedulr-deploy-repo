#!/usr/bin/env python3
"""
Wrapper for primeVersion2.py that accepts JSON input and returns 
JSON output
This preserves all the original logic and constraints from the 
fine-tuned scheduler
"""
...
def months_since(start_date, end_date):
    """
    Return the number of whole months between start_date and 
end_date.
    """
...
# --------------------------------------------------------------------------------
def check_standby_pairing_feasibility(availability, date_list, 
doctors):
    """Check if Saturday/Sunday pairing is possible for Standby 
Oncall"""
    warnings = []
    ...
    for i in range(len(date_list)-1):
        ...
        if current_date.weekday() == 5 and next_date.weekday() == 6:
# Sat->Sun
            sat_doctors = set()
            sun_doctors = set()

            for d in doctors:
                if availability.get((d, i, 'Standby Oncall'),
False):
                    sat_doctors.add(d)
                if availability.get((d, i+1, 'Standby Oncall'),
False):
                    sun_doctors.add(d)
            ...
            if not intersection:
                ...
                logger.info(f"    Saturday available: 
{sorted(sat_doctors)}")
                logger.info(f"    Sunday available: 
{sorted(sun_doctors)}")
            else:
...
def run_prime_scheduler(config: Dict[str, Any]) -> Dict[str, Any]:
    """
    Main function that runs the prime scheduler with JSON 
input/output
    ...
    Args:
        config: Dictionary containing:
            ...
            - solver_config: Solver parameters (lambda weights, 
etc.)
    ...
    try:
        ...
        for doc in config['doctors']:
            ...
            doctor_info[doc['id']] = {
                ...
                "last_standby": datetime.datetime.strptime(doc['last_standby'], '%Y-%m-%d').date()
if doc['last_standby'] else None,
                ...
                if current_date.weekday() == 5 and next_date.weekday() == 6:  # Sat->Sun
                    ...
                    if both_days_doctors and standby_sat_vars and standby_sun_vars:
                        ...
                        matched_assignments = cp.sum([
                            x[d, s, "Standby Oncall"] * x[d, s+1,"Standby Oncall"] for d in D if (d, s, "Standby Oncall") in x and (d,s+1, "Standby Oncall") in x]) if standby_sat_vars and standby_sun_vars else 0

                        # Penalty if both days have assignments but different doctors
                        # mismatch_penalty = 1 if (total_sat > 0 andtotal_sun > 0 and matched_assignments == 0)
                        if isinstance(total_sat, (int, float)) and isinstance(total_sun, (int, float)):
                            ...
                            if total_sat > 0 and total_sun > 0:
                                constraints.append(mismatch_penalty
>= 1 - matched_assignments)
                        else:
                            # Both are variables
...
posts_by_day[s+1])
                        ...
                        if is_standby_weekend:
                            # FIXED: Soft constraint for Standby 
Oncall weekend pairs
                            standby_today = [x[d, s, "Standby 
Oncall"]] if (d, s, "Standby Oncall") in x else []
                            standby_tomorrow = [x[d, s+1, "Standby 
Oncall"]] if (d, s+1, "Standby Oncall") in x else []

                            if standby_today and standby_tomorrow
and (d, s) in rest_violation:
                                # Soft rest constraint: 
standby_today + standby_tomorrow <= 1 + violation
                                constraints.append(
                                    cp.sum(standby_today) +
cp.sum(standby_tomorrow) <= 1 + rest_violation[d, s]
                                )

penalty_terms.append(STANDBY_REST_PENALTY_WEIGHT * rest_violation[d,
s])

                            # Other oncall posts still have hard 
constraint
                            other_today = [x[d, s, t] for t in
posts_by_day[s]
                                        if t in oncall_posts and t
!= "Standby Oncall" and (d, s, t) in x]
                            other_tomorrow = [x[d, s+1, t] for t in
posts_by_day[s+1]
                                            if t in oncall_posts
and t != "Standby Oncall" and (d, s+1, t) in x]

                            if other_today and other_tomorrow:

constraints.append(cp.sum(other_today) + cp.sum(other_tomorrow) <=
1)
                        else:
                            # Regular rest constraint for 
non-Standby posts
                            if (d, s) in rest_violation:
                                constraints.append(
                                    cp.sum(oncall_today) +
cp.sum(oncall_tomorrow) + rest_violation[d, s] >= 1
                                )
                                penalty_terms.append(lambda_rest *
rest_violation[d, s])
                            else:
                                # Hard constraint if no rest 
violation var

constraints.append(cp.sum(oncall_today) + cp.sum(oncall_tomorrow) <=
1)
...
penalty_terms.append(lambda_after_clinic * x[d, idx, t])
            ...
            for (d, s, t) in x.keys():
                if (doctor_info[d]["category"] == "registrar"
                    ...
                    penalty_terms.append(lambda_reg_weekend * x[d,
s, t])
            ...
            for (d, s, t) in x.keys():
                if (doctor_info[d]["category"] == "junior"
                    ...
                    penalty_terms.append(lambda_junior_ward * x[d,
s, t])

            # ED assignment penalties (seniors/registrars prefer not
to do ED)
            for (d, s, t) in x.keys():
                if (doctor_info[d]["category"] in ["senior",
"registrar"]
                    and t.startswith("ED")):
            ...
            for d in D:
                if doctor_info[d]["category"] != "floater" and d in
min_one_slack:
                    total_assignments = cp.sum([x[d, s, t] for s in
S for t in posts_by_day[s] if (d, s, t) in x])
                    constraints.append(total_assignments +
min_one_slack[d] >= 1)
                    penalty_terms.append(lambda_min_one *
min_one_slack[d])
            ...
            for d in D:
                for s in range(len(date_list) - 2):
                    if (d, s) in z_gap:
                        oncall_today = [x[d, s, t] for t in
posts_by_day[s] if t in oncall_posts and (d, s, t) in x]
                        oncall_plus3 = [x[d, s+2, t] for t in
posts_by_day[s+2] if t in oncall_posts and (d, s+2, t) in x]

                        if oncall_today and oncall_plus3:
                            # z_gap[d,s] = 1 if both today and +2 
days have oncall assignments
                            constraints.append(z_gap[d, s] >=
cp.sum(oncall_today) + cp.sum(oncall_plus3) - 1)
                            penalty_terms.append(-lambda_gap *
z_gap[d, s])  # Negative = reward
            ...
            for u in units:
                ...
                if len(unit_docs) > 0:
                    ...
                    for s, date in enumerate(date_list):
                        if date.weekday() not in clinic_days.get(u,
[]):  # Non-clinic days
                            unit_assignments = [x[d, s, t] for d in
unit_docs for t in posts_by_day[s] if (d, s, t) in x]
                            if unit_assignments:
                                over_slack =
...
----------------------
        ...
        if problem1.status == cp.OPTIMAL:
            logger.info("Phase 1 succeeded - using optimal 
solution")
            final_problem, final_x, final_mismatch = problem1, x1,
mismatch1
        else:
            logger.info("Phase 1 failed - running Phase 2...")
            problem2, x2, mismatch2 = build_and_solve(RELAX=True)
            final_problem, final_x, final_mismatch = problem2, x2,
mismatch2

        # === EXTRACT RESULTS ===
        results = []
        actual_pairing_relaxed = pairing_relaxed  # Start with 
pre-solve assessment

        if final_problem.status in [cp.OPTIMAL,
cp.OPTIMAL_INACCURATE]:
            # Check if any mismatch penalties were activated
            for penalty_var in final_mismatch.values():
                if penalty_var.value is not None and
penalty_var.value > 0.5:
                    actual_pairing_relaxed = True
                    logger.warning("🔄 Standby Oncall pairing was 
relaxed due to solver constraints")
                    break

            # Extract assignments (NO special Standby Oncall 
handling - let post-processing handle it)
...
Binary variable threshold
            ...
            # Add clinic assignments for all doctors in units on 
clinic days
            for u, weekdays in clinic_days.items():
                unit_docs = [d for d in D if doctor_info[d]["unit"]
== u]
                for s, date in enumerate(date_list):
            ...
            logger.info(f"Generated schedule with {len(results)} 
assignments")

            # Log Standby Oncall assignments specifically
            standby_assignments = [r for r in results if r["post"]
== "Standby Oncall"]
            logger.info(f"Standby Oncall assignments: 
{len(standby_assignments)}")
            for assignment in standby_assignments:
                date_obj =
datetime.datetime.strptime(assignment["date"], '%Y-%m-%d').date()
                weekday_name = date_obj.strftime('%A')
                logger.info(f"  {assignment['doctor']} -> 
{assignment['date']} ({weekday_name})")

        else:
            logger.warning(f"Solver failed with status: 
{final_problem.status}")

        # === CALCULATE STATISTICS ===
        stats = {
            "total_assignments": len(results),
            "doctors_used": len(set(item["doctor"] for item in
results)),
            "posts_filled": {},
            "assignments_by_date": {},
            "workload_by_doctor": {},
            "solver_status": str(final_problem.status),
            "objective_value": final_problem.value if
final_problem.value is not None else None
        }
        ...
        return {
            ...
            "objective_value": final_problem.value if
final_problem.value is not None else None,
            "success": final_problem.status in [cp.OPTIMAL,
cp.OPTIMAL_INACCURATE],
